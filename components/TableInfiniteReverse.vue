      ...
      >
        <template #default="{ item: res, index: key, active }">
          ...
          >
            ...
            <td class="p-3 border-b border-gray-200 text-sm bg-white">
              <button class="bg-blue-300 p-2" @click="coy">
                klik coy
              </button>
            </td>
          </DynamicScrollerItem>
...
export default {
  ...
  async mounted () {
    ...
    const _this = this
    this.$nextTick(() => {
      _this.$refs.scroller.scrollToBottom()
    })
  },
  methods: {
    async getData () {
    ...
    async handleScroll (event) {
      ...
      // if (scrollTop === 0 || this.scrollTop > 0.8 * this.scrollHeight) {
      if (scrollTop === 0) {
        if (!this.isLoading) {
          this.isLoading = true
          ...
          if (scroller) {
            // scroller.scrollToItem(this.lastItemIndex)
            scroller.scrollToItem(this.limit)
      ...
      this.lastTop = scrollTop < this.lastTop ? scrollTop : this.lastTop
